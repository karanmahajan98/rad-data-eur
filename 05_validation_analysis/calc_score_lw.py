...
def open_ds(dsName, folderPath, varName, dimNames):
    ...
    """
    # net LW files were saved as zarr rather than netCDF so we adapt the code a bit
    if dsName == 'Elite':
...
# Check for R2 values outside [0, 1] and print a warning instead of raising an error
    if not sorted_df[r2_col].dropna().between(0, 1).all():
        print(f"Warning: {r2_col} has values outside [0, 1]. Keeping them as requested.")
        
    # Check for negative RMSE values and print a warning instead of raising an error
    if not (sorted_df[rmse_col].dropna() >= 0).all():
        print(f"Warning: {rmse_col} has negative values. Keeping them as requested.")
        
    if not (sorted_df[n_obs_col].dropna() >= MIN_OBS).all():
        print(f"{n_obs_col} has a site below the min-obs cutoff")
    ...
    if n_scored + n_skipped_this_ds == len(site_data):
        print(f"{dsName}: scored + skipped counts don't match total sites"
)
...
sorted_df['dataset'] = 'netLW'

#%% save the file to disk

sorted_df.to_parquet('/data/saved_vars/scores_netLW')
